...
namespace Web.Controllers.ApiControllers
{
    ...
    [ApiController]
    public class Old_OwnersApiEndpoint : ControllerBase
    {
        private readonly IUnitOfWork _unitOfWork;

        public Old_OwnersApiEndpoint(IUnitOfWork unitOfWork)
        {
        ...
        public IActionResult Delete(int id)
        {
            if (OwnerExists(id))
            {
                _unitOfWork.Owners.Remove(_unitOfWork.Owners.Find(id));
                _unitOfWork.Complete();
                return NoContent();
            }
            return NotFound(id);
